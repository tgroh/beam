...
class SdkComponents {
  ...
  public void validate() {
    Set<String> missingTransforms = new HashSet<>();
    for (String transformId : transformIds.values()) {
      if (componentsBuilder.getTransformsOrDefault(transformId, null) == null) {
        missingTransforms.add(transformId);
      }
    }
    checkState(
        missingTransforms.isEmpty(),
        "%s transforms were registered but were never added to components. Missing transforms %s",
