...
public class AssertionCountingVisitorTest {
  ...
  public void getAssertCountCountsAsserts() {
    PCollection<Integer> create = pipeline.apply(Create.of(1, 2, 3));

    PAssert.that(create).containsInAnyOrder(1, 2, 3);
    PAssert.thatSingleton(create.apply(Sum.integersGlobally())).equals(6);
    PAssert.thatMap(pipeline.apply(Create.of(KV.of(1, 2)))).equals(Collections.singletonMap(1, 2));
