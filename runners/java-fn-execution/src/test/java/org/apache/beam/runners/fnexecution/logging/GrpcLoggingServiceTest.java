...
public class GrpcLoggingServiceTest {
  ...
  public void testMultipleClientsSuccessfullyProcessed() throws Exception {
    ConcurrentInMemoryLogWriter logs = new ConcurrentInMemoryLogWriter();
    final GrpcLoggingService service = GrpcLoggingService.create(logs);
      ...
      for (int i = 1; i <= 3; ++i) {
        ...
        tasks.add(new Callable<Void>() {
          ...
          public Void call() throws Exception {
            ...
            StreamObserver<List> outboundObserver =
                BeamFnLoggingGrpc.newStub(channel).logging(
                    TestStreams.withOnNext(messageDiscarder)
  ...
  public void testServerCloseHangsUpClients() throws Exception {
    ...
    GrpcLoggingService service = GrpcLoggingService.create(logs);
    try (GrpcFnServer<GrpcLoggingService> server =
        GrpcFnServer.allocatePortAndCreateFor(service, InProcessServerFactory.create())) {
  ...
  private static class ConcurrentInMemoryLogWriter implements LogWriter {
    private final LinkedBlockingQueue<BeamFnApi.LogEntry> entries = new LinkedBlockingQueue<>();
