...
/**
 * A {@link Runnable} that implements the work dispatch loop for the DirectRunner.
 */
public class WorkLoopRunnable implements Runnable {
  private static final Logger LOG = LoggerFactory.getLogger(WorkLoopRunnable.class);
  ...
  public void run() {
    ...
    try {
      ...
      ExecutorState startingState = state;
      if (startingState == ExecutorState.ACTIVE) {
        ...
        state = ExecutorState.PROCESSING;
  ...
  private void signalMonitorDeath(Exception t) {
  }
  ...
  void workProduced(CommittedBundle<?> bundle, Iterable<AppliedPTransform<?, ?, ?>> consumers) {}
  ...
  abstract static class ExecutorUpdate {
    public static ExecutorUpdate fromBundle(
      ...
      return new AutoValue_WorkLoopRunnable_ExecutorUpdate(
          Optional.of(bundle),
          consumers,
