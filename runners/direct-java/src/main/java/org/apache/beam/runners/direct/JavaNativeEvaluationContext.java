 ...
 *
 * <p>{@link JavaNativeEvaluationContext} contains shared state for an execution of the {@link
 * DirectRunner} that can be used while evaluating a {@link PTransform}. This consists of views into
 * underlying state and watermark implementations, access to read and write {@link PCollectionView
 ...
 *
 * <p>{@link JavaNativeEvaluationContext} also handles results by committing finalizing bundles
 * based on the current global state and updating the global state appropriately. This includes
 * updating the per-{@link StepAndKey} state, updating global watermarks, and executing any
...
class JavaNativeEvaluationContext
  ...
  public static JavaNativeEvaluationContext create(
      Clock clock,
      BundleFactory bundleFactory,
      ExecutableGraph<AppliedPTransform<?, ?, ?>, PValue> graph,
      Set<PValue> keyedPValues,
      Collection<PCollectionView<?>> views) {
    return new JavaNativeEvaluationContext(clock, bundleFactory, graph, keyedPValues, views);
  }

  private JavaNativeEvaluationContext(
      Clock clock,
      BundleFactory bundleFactory,
      ExecutableGraph<AppliedPTransform<?, ?, ?>, PValue> graph,
      Set<PValue> keyedPValues,
