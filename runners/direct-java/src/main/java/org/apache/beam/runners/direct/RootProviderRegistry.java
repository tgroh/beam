...
class RootProviderRegistry {
   ...
   */
  public static RootProviderRegistry impulseRegistry(EvaluationContext ctxt) {
    return new RootProviderRegistry(
        ImmutableMap.<String, RootInputProvider<?, ?, ?>>builder()
            .put(IMPULSE_TRANSFORM_URN, new ImpulseEvaluatorFactory.ImpulseRootProvider(ctxt))
            .build());
  }
