...
interface TransformEvaluatorFactory<ExecutableT> {
  ...
  <InputT> TransformEvaluator<InputT> forApplication(
      ExecutableT executable, CommittedBundle<?> inputBundle) throws Exception;

  /**
   * Cleans up any state maintained by this {@link TransformEvaluatorFactory}. Called after a {@link
   * Pipeline} is shut down. No more calls to {@link #forApplication(ExecutableT, CommittedBundle)}
   * will be made after a call to {@link #cleanup()}.
   */
