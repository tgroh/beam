...
public class WatermarkManager {
  ...
  public class TransformWatermarks {
    ...
    private Collection<FiredTimers> extractFiredTimers() {
      ...
      for (Map.Entry<StructuralKey<?>, List<TimerData>> firedTimers :
        ...
        keyFiredTimers.add(
            FiredTimers.create(transform, firedTimers.getKey(), firedTimers.getValue()));
      }
  ...
  /**
   * A triple of {@link TimerData timers}, {@link StructuralKey key}, and {@link AppliedPTransform}
   * which set the timers. A timer fires at the transform that set it with a specific key when the
   * time domain in which it lives progresses past a specified time, as determined by the {@link
   * WatermarkManager}.
   */
  @AutoValue
  public abstract static class FiredTimers {
    private static FiredTimers create(
        AppliedPTransform<?, ?, ?> transform, StructuralKey<?> key, Collection<TimerData> timers) {
      return new AutoValue_WatermarkManager_FiredTimers(transform, key, timers);
    }
    ...
    abstract Collection<TimerData> getTimers();
  }
