...
public class WatermarkManager {
   ...
   */
  public Map<AppliedPTransform<?, ?, ?>, Map<StructuralKey<?>, KeyedWorkItem<?, ?>>>
      extractFiredTimers() {
    Map<AppliedPTransform<?, ?, ?>, Map<StructuralKey<?>, KeyedWorkItem<?, ?>>> allTimers =
        new HashMap<>();
    for (Map.Entry<AppliedPTransform<?, ?, ?>, TransformWatermarks> watermarksEntry :
        transformToWatermarks.entrySet()) {
      Map<StructuralKey<?>, KeyedWorkItem<?, ?>> fired =
  ...
  public class TransformWatermarks {
    ...
    private Map<StructuralKey<?>, KeyedWorkItem<?, ?>> extractFiredTimers() {
      ...
      Map<StructuralKey<?>, KeyedWorkItem<?, ?>> keyFiredTimers = new HashMap<>();
      for (Map.Entry<StructuralKey<?>, Map<TimeDomain, List<TimerData>>> firedTimers :
          groupedTimers.entrySet()) {
        KeyedWorkItem<?, ?> timerWorkItem =
            KeyedWorkItems.timersWorkItem(
                firedTimers.getKey().getKey(), Iterables.concat(firedTimers.getValue().values()));
