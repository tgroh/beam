...
public class TransformExecutorTest {
  ...
  private AppliedPTransform<PBegin, PCollection<String>, Values<String>> createdProducer;

  private PCollection<KV<Integer, String>> downstream;
  private AppliedPTransform<
          PCollection<String>, PCollection<KV<Integer, String>>, WithKeys<Integer, String>>
      downstreamProducer;
  ...
  public void setup() {
    ...
    Create.Values<String> createValues = Create.of("foo", "spam", "third");
    created = p.apply(createValues);
    createdProducer = AppliedPTransform.of("Create", p.begin(), created, createValues);
    WithKeys<Integer, String> withKeys = WithKeys.<Integer, String>of(3);
    downstream = created.apply(withKeys);
    downstreamProducer = AppliedPTransform.of("WithKeys", created, downstream, withKeys);
  ...
  public void callWithNullInputBundleFinishesBundleAndCompletes() throws Exception {
    ...
    when(registry.forApplication(createdProducer, null))
  ...
  public void inputBundleProcessesEachElementFinishesAndCompletes() throws Exception {
    ...
    when(registry.<String>forApplication(downstreamProducer, inputBundle))
        .thenReturn(evaluator);
  ...
  public void callWithEnforcementAppliesEnforcement() throws Exception {
    ...
    when(registry.forApplication(downstreamProducer, inputBundle))
        .thenReturn(evaluator);
  ...
  public void callWithEnforcementThrowsOnFinishPropagates() throws Exception {
    ...
    CommittedBundle<KV<Integer, String>> inputBundle =
        bundleFactory.createBundle(downstreamProducer.getOutput()).commit(Instant.now());
    when(registry.forApplication(downstreamProducer, inputBundle)).thenReturn(evaluator);

    TransformExecutor<KV<Integer, String>> executor =
        TransformExecutor.create(
            evaluationContext,
            registry,
            Collections.<ModelEnforcementFactory>singleton(
                new ThrowingEnforcementFactory(ThrowingEnforcementFactory.When.AFTER_FINISH)),
            inputBundle,
            downstreamProducer,
            completionCallback,
            transformEvaluationState);
  ...
  public void callWithEnforcementThrowsOnElementPropagates() throws Exception {
    ...
    WindowedValue<KV<Integer, String>> theKv =
        WindowedValue.valueInGlobalWindow(KV.of(1, "one"));
    CommittedBundle<KV<Integer, String>> inputBundle =
        bundleFactory
            .createBundle(downstream)
            .add(theKv)
            .commit(Instant.now());
    when(registry.forApplication(downstreamProducer, inputBundle))
        .thenReturn(evaluator);
  ...
  private static class ThrowingEnforcementFactory implements ModelEnforcementFactory {
    ...
    public <T> ModelEnforcement<T> forBundle(
        CommittedBundle<T> input, AppliedPTransform<?, ?, ?> consumer) {
      return new ModelEnforcement<T>() {
        @Override
        public void beforeElement(WindowedValue<T> element) {
          if (when == When.BEFORE_ELEMENT) {
            throw new RuntimeException(when.toString());
          }
        }
