...
public class ModelEnforcingTransformEvaluatorFactory implements TransformEvaluatorFactory {
  ...
  public static ModelEnforcingTransformEvaluatorFactory of(
      TransformEvaluatorFactory wrapped, Collection<ModelEnforcementFactory> enforcements) {
    return new ModelEnforcingTransformEvaluatorFactory(wrapped, ImmutableList.copyOf(enforcements));
  }

  private ModelEnforcingTransformEvaluatorFactory(
  ...
  private static class ModelEnforcingTransformEvaluator<T> implements TransformEvaluator<T> {
    ...
    public void processElement(WindowedValue<T> element) throws Exception {
      for (ModelEnforcement<T> enforce : enforced) {
        enforce.beforeElement(element);
      }
      wrapped.processElement(element);
      for (ModelEnforcement<T> enforce : enforced) {
        enforce.afterElement(element);
      }
    }
