...
public class PCollectionViews {
  ...
  private static class SimplePCollectionView<ElemT, ViewT, W extends BoundedWindow>
    ...
    public void verifyCompatible(PValue other) {
      checkArgument(
          other instanceof PCollectionView,
          "%s is not a %s",
          other,
          PCollectionView.class.getSimpleName());
      PCollectionView<?> that = (PCollectionView<?>) other;
      checkArgument(
          this.getTagInternal().equals(that.getTagInternal()),
          "Internal %s mismatch: %s vs %s",
          getTagInternal().getClass().getSimpleName(),
          this.getTagInternal(),
          that.getTagInternal());
      checkArgument(
          this.getCoderInternal().equals(that.getCoderInternal()),
          "%s mismatch: %s vs %s",
          Coder.class.getSimpleName(),
          this.getCoderInternal(),
          that.getCoderInternal());
      checkArgument(
          this.getWindowingStrategyInternal().equals(that.getWindowingStrategyInternal()),
          "%s mismatch: %s vs %s",
          WindowingStrategy.class.getSimpleName(),
          this.getWindowingStrategyInternal(),
          that.getWindowingStrategyInternal());
      checkArgument(
          this.getViewFn().equals(that.getViewFn()),
          "%s mismatch: %s vs %s",
