...
class InProcessSideInputContainer {
  ...
  private final class SideInputContainerSideInputReader implements ReadyCheckingSideInputReader {
    ...
    private final ConcurrentMap<
        PCollectionViewWindow<?>, Optional<Iterable<? extends WindowedValue<?>>>>
        viewContents;
    ...
    private SideInputContainerSideInputReader(Collection<PCollectionView<?>> readerViews) {
      ...
      this.viewContents = new ConcurrentHashMap<>();
    }
    ...
    public boolean isReady(final PCollectionView<?> view, final BoundedWindow window) {
      checkArgument(
          ...
          view,
          readerViews);
      return getViewContents(view, window).isPresent();
    }
    ...
    public <T> T get(final PCollectionView<T> view, final BoundedWindow window) {
      ...
      @SuppressWarnings("unchecked") Iterable<WindowedValue<?>> values =
          (Iterable<WindowedValue<?>>) getViewContents(view, window).get();
      return view.fromIterableInternal(values);
    }

    /**
     * Gets the contents containing the contents of the provided {@link PCollectionView} in the
     * provided {@link BoundedWindow}, setting up a callback to populate the contents in the
     * enclosing container if necessary.
     */
    @Nullable
    private synchronized <T> Optional<Iterable<? extends WindowedValue<?>>> getViewContents(
      ...
      if (viewContents.containsKey(windowedView)) {
        return viewContents.get(windowedView);
      }
      final AtomicReference<Iterable<? extends WindowedValue<?>>> contents =
          viewByWindows.getUnchecked(windowedView);
      ...
      evaluationContext.scheduleAfterOutputWouldBeProduced(
          view, window, windowingStrategy, new WriteEmptyViewContents(view, window, contents));

      viewContents.putIfAbsent(windowedView,
          Optional.<Iterable<? extends WindowedValue<?>>>fromNullable(contents.get()));
      return viewContents.get(windowedView);
    }
