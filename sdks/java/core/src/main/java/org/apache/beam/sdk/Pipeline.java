...
import org.apache.beam.sdk.runners.PTransformMatcher.Match;
...
public class Pipeline {
  ...
  public void traverseTopologically(PipelineVisitor visitor) {
    ...
    if (!visitedValues.containsAll(values)) {
      List<PValue> notVisited = new ArrayList<>(values);
      notVisited.removeAll(visitedValues);
      throw new IllegalStateException(
          String.format(
              "Should have visited all of the values after visiting all the transforms. "
                  + "Not visited: %s",
              notVisited));
    }
  }
  ...
  /**
   * Replace all of the PTransforms that match the provided {@link PTransformMatcher} with a {@link
   * PTransform} generated by the {@link PTransformFactory}.
   *
   * @return true if any {@link PTransform} was replaced, and false otherwise.
   */
  public boolean replaceMatches(final PTransformMatcher filter, final PTransformFactory factory) {
    final Map<TransformTreeNode, PTransform> matches = new HashMap<>();
    transforms.visit(new PipelineVisitor.Defaults() {
      @Override
      public CompositeBehavior enterCompositeTransform(TransformTreeNode node) {
        if (node.getTransform() == null) {
          return CompositeBehavior.ENTER_TRANSFORM;
        }
        Match match = filter.match(node.getTransform());
        switch(match) {
          case REPLACE:
            addMatch(node);
          ...
          case CONTINUE:
            return CompositeBehavior.ENTER_TRANSFORM;
          default:
            throw new IllegalArgumentException(String.format("Unknown match type %s", match));
        }
      }

      @Override
      public void visitPrimitiveTransform(TransformTreeNode node) {
        if (filter.match(node.getTransform()) == Match.REPLACE) {
          addMatch(node);
        }
      }

      private void addMatch(TransformTreeNode node) {
        PTransform replacement = factory.create(node.getTransform());
        if (replacement != null && !replacement.equals(node.getTransform())) {
          matches.put(node, replacement);
        }
      }
    }, new HashSet<PValue>());
    CheckEnabled stableUniqueNames = getOptions().getStableUniqueNames();
    getOptions().setStableUniqueNames(CheckEnabled.OFF);
    for (Map.Entry<TransformTreeNode, PTransform> match : matches.entrySet()) {
      this.replace(match.getKey(), match.getValue());
    }
    getOptions().setStableUniqueNames(stableUniqueNames);
  ...
  private <InputT extends PInput, OutputT extends POutput> void replace(
    ...
    try {
      ...
      // These are added to the Pipeline, but are transient and must be removed
      OutputT newOutput = runner.apply(replacement, input);
      values.removeAll(newOutput.expand());
      AppliedPTransform<?, ?, ?> replacementApplication =
          AppliedPTransform.of(replacementNode.getFullName(), input, output, replacement);
      output.replaceAsOutput(originalApplication, replacementApplication);
      if (output instanceof PValue) {
        checkNotNull(((PValue) output).getProducingTransformInternal());
      }
