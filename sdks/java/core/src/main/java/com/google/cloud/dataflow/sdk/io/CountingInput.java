 ...
 * {@link IsBounded#UNBOUNDED unbounded} {@link PCollection}, {@link CountingInput} starts at
 * {@code 0} and counts up to {@link Long#MAX_VALUE}, and then never produces more output. (In
...
public class CountingInput {
  ...
  public static class BoundedCountingInput extends PTransform<PBegin, PCollection<Long>> {
    ...
    public PCollection<Long> apply(PBegin begin) {
      return begin.apply(Read.from(new BoundedCountingSource(0L, numElements)));
    }
  }
      ...
      Unbounded<Long> read =
          Read.from(
              new UnboundedCountingSource(
                  0L /* start */, 1L /* stride */, elementsPerPeriod, period, timestampFn));
  ...
  /**
   * A {@link BoundedSource} that produces a {@link PCollection} of {@link Long Longs}.
   ...
   */
  @VisibleForTesting
  static class BoundedCountingSource extends OffsetBasedSource<Long> {
    /**
  ...
  /**
   * An {@link UnboundedSource} that produces a {@link PCollection} of {@link Longs}.
   */
  @VisibleForTesting
  static class UnboundedCountingSource extends UnboundedSource<Long, CounterMark> {
     ...
     */
    @Override
    public List<? extends UnboundedSource<Long, CounterMark>> generateInitialSplits(
  ...
  /**
   * The checkpoint for an {@link UnboundedCountingSource} is the last value produced and the
   * start time of the {@link UnboundedReader}. The associated source object encapsulates the
   * information needed to produce the next value.
   */
